...
| §13, In-process API and graph observer feed | `api.md`. Producer-side observer publication guarantees remain in the relevant processor document and are referenced by `api.md`. | Extracted; BlockProcessor producer ownership reconciled, VspcProcessor and final verification pending |
