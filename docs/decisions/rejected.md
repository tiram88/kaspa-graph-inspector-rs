...
| Derive a destination from a VSPC change with a nonempty removed chain and an empty added path. | The shape violates the selected-sink invariant; [VSPC processing](../architecture/vspc-processing.md#change-semantics--settled) owns the absence of a fallback destination and links to each source-specific owner. |
